...
/**
 * This is an implementation of DelayedConstantConnection statemachine. 
 * To use it from an active element do the following:
 *
 * <ul>
 * <li>call setStatemachine(new DelayedConstantConnectionSTM(this)) from the constructor.</li>
 *
 ...
 */
public class DelayedConstantConnectionSTM extends Statemachine {
	private static final int ST_start = 0;
	private static final int ST_stop = 1;
	private static final int ST_Idle = 2;
	private static final int ST_Transmit = 3;

	// A list of action constants
	public static final int ACTION_hasSignals = 0;
	...
	public static final String STATE_Transmit = "Transmit";

	private int currentState;
	private GSignal activeSignal;
	private boolean isFinished = false;

	public DelayedConstantConnectionSTM(ActiveElement parent) {
		super(parent);
		activeSignal = null;
	...
	private void enterState(int state) throws Exception {
		switch (state) {
			...
			case ST_Transmit :
				...
				getParent().action(ACTION_transmit);

				// set timers
				// set state name
				setCurrentStateName(STATE_Transmit);
				break;
