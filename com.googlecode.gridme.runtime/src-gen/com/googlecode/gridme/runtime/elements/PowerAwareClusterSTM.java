...
/**
 * This is an implementation of PowerAwareCluster statemachine. 
 * To use it from an active element do the following:
 *
 * <ul>
 * <li>call setStatemachine(new PowerAwareClusterSTM(this)) from the constructor.</li>
 *
 ...
 */
public class PowerAwareClusterSTM extends Statemachine {
	...
	private static final int ST_Hourly = 5;
	private static final int ST_PowerManage = 6;

	// A list of action constants
	public static final int ACTION_getScheduleDelay = 0;
	public static final int ACTION_doHourly = 1;
	public static final int ACTION_doSchedule = 2;
	public static final int ACTION_powerManage = 3;
	...
	public static final String STATE_PowerManage = "PowerManage";
	...
	public PowerAwareClusterSTM(ActiveElement parent) {
		super(parent);
		activeSignal = null;
		...
		do {
			...
			switch (currentState) {
				...
				case ST_Idle :
					...
					activeSignal = getParent()
							.checkSignal(
									com.googlecode.gridme.runtime.elements.GTaskFinishedSignal.class);
					if (activeSignal != null) {
						if (true) {
							leaveState(ST_Idle);
					...
					activeSignal = getParent()
							.checkSignal(
									com.googlecode.gridme.runtime.elements.GNodeFreeSignal.class);
					if (activeSignal != null) {
						if (true) {
							leaveState(ST_Idle);
					...
					activeSignal = getParent()
							.checkSignal(
									com.googlecode.gridme.runtime.elements.GPowerManageSignal.class);
					if (activeSignal != null) {
						if (true) {
							leaveState(ST_Idle);
	...
	private void enterState(int state) throws Exception {
		switch (state) {
			...
			case ST_Schedule :
				...
				getParent().action(ACTION_doSchedule);
			...
			case ST_PowerManage :
				...
				getParent().action(ACTION_powerManage);

				// set timers
				// set state name
				setCurrentStateName(STATE_PowerManage);
				break;
