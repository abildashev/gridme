...
@ImplementationDescription(value = "The tasks in queue are ordered by the priority in decreasing order. "
    ...
    + "of SQR/W is minimal. SQR - sum of all task squares (using real execution time), "
    + "W - cluster size.", category = ImplementationDescription.SCHED_GLOBAL)
public class SqrWMinGlobal extends BaseBestFitGlobal
{
  public SqrWMinGlobal()
  {
  ...
  {
    return (float) (cluster.getScheduler().getQueue().getRealSquare() + task
        .getNodesMin()
