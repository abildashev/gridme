...
@ImplementationDescription(value = "The tasks in queue are ordered by the priority in decreasing order. "
    ...
    + "of SQR/W is minimal. SQR - sum of all task squares (using requested execution time), "
    + "W - cluster size.", category = ImplementationDescription.SCHED_GLOBAL)
public class SqrExpWMinGlobal extends BaseBestFitGlobal
{
  public SqrExpWMinGlobal()
  {
  ...
  {
    return (float) (cluster.getScheduler().getQueue().getExpectedSquare() + task
        .getNodesMin()
